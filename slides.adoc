...
[%step]
* Poker white label __Betclic__ 🏷️
* Propre plateforme 🆕
...
* 🆕 Plateforme de Poker & App. cross-devices 
* *09/2023*
** Lancement du projet 🚀
...
[%step]
* Sur l'étagère 📦 (SaaS, PaaS)
* Archi µservices hexa (mix DDD)
* `Kotlin` + `Ktor`
* Immutabilité 🪨
* API first 📃🙏
* CQRS + Events 📧
...
class Table(
  ...
  val config: TableConfig,
   ...
   fun seat(player: Player) {
      with(config) {
        check(players.size in minPlayers..maxPlayers) <1>
      }
      <2>
      check(players.distinctBy { it.position }.size == players.size)
      players.add(player) // mutation
   }
...
data class Table(
   ...
   fun seat(player: Player): Table =
      copy(players = players + player) // immutable

   fun lastPlayerLeaves(): Table =
      copy(players = players.dropLast(1)) // immutable
}
----
...
value class Chips(val value: Long) : Comparable<Chips> {
  ...
  companion object {
    val Int.chips get() = Chips(toLong())
  }
}
----
...
123.chips + 42.chips == 165.chips // true
111.chips * 7 == 777.chips        // true
123.chips - 888.chips // 💥 Chips cannot be negative
----
...
value class Chips(val value: Long) : Comparable<Chips> {
  ...
  operator fun div(divider: Int): Chips = throw NotImplementedError()

  infix fun splitBy(divider: Int): WholeAndRemainder {
    require(divider > 0)
...
[%step]
* _Tournois_ Enreg. tardif de masse 👨‍👩‍👦‍👦⌚
* _CashGame_ sélection des adversaires 🔍
* _Spin&Rush_ lancements en rafale 📈
...
* Observabilité
* KPIs métier
* Système de bots interne (tests)
* UX/Designers, QA tests autos/manuels 😘
